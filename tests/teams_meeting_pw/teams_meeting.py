...
How it works:
    Copies only the cookie/session files from your Chrome profile to a temp
    directory, then opens that via launch_persistent_context. This loads your
    existing Microsoft session so Power Automate opens without login redirect,
    and works even while Chrome is open (no lock conflict on the temp copy).
    The temp directory is deleted automatically when the script finishes.
"""

# ── Standard library — all imports at top level (OPT-1, OPT-7, OPT-10) ──────
import argparse
import ctypes
...
# ── Win32 constants for locked-file copy (OPT-4) ─────────────────────────────
# Initialised once at module level instead of inside _copy_file_locked() on
# every call. On non-Windows these are defined but never used.
_IS_WINDOWS        = platform.system() == "Windows"
_GENERIC_READ      = 0x80000000
_FILE_SHARE_RWD    = 0x00000001 | 0x00000002 | 0x00000004  # READ|WRITE|DELETE
_OPEN_EXISTING     = 3
...
def get_version_schedule(config: dict, version: str) -> dict:
    ...
    Supports both nested (meeting_dates sub-key) and flat config structures.
    Called once per run instead of once per meeting.
    """
...
def get_meeting_date(config: dict, version: str, meeting_name: str) -> str | None:
    ...
    return get_version_schedule(config, version).get(meeting_name)
...
def _copy_file_locked(src: str, dst: str) -> bool:
    ...
    exclusive write lock. We use Win32 CreateFileW with FILE_SHARE_WRITE,
    which allows reading even while another process holds a write lock.

    Win32 constants and the kernel32 handle are module-level (OPT-4) so this
    function pays no initialisation cost on repeated calls.
    """
    if not _IS_WINDOWS:
        try:
    ...
    try:
        h = _kernel32.CreateFileW(
            src, _GENERIC_READ, _FILE_SHARE_RWD,
        ...
        with open(dst, "wb") as out:
            while True:
                ok = _kernel32.ReadFile(h, buf, CHUNK, ctypes.byref(bytes_read), None)
...
def _copy_profile(src_dir: str) -> str:
    ...
    Only Default/Network/Cookies and Local State are copied (a few MB).
    Everything else (extensions, cache, history) is left out.
    The temp directory is deleted automatically by close_browser().
    """
    ...
    for rel in session_files:
        ...
        if os.path.isfile(src_f):
            ...
            (copied if _copy_file_locked(src_f, dst_f) else skipped).append(rel)

    # Local State holds the DPAPI key needed to decrypt cookies on Windows
    local_state = os.path.join(src_dir, "Local State")
    if os.path.isfile(local_state):
        dst = os.path.join(tmp, "Local State")
        (copied if _copy_file_locked(local_state, dst) else skipped).append("Local State")

    if copied:
        print(f"  Copied  : {', '.join(copied)}")
    if skipped:
        print(f"  ⚠ Skipped: {', '.join(skipped)} — session may require login")
...
def setup_browser(playwright, profile_path: str = None) -> tuple[BrowserContext, Page]:
    """
    Launch Chrome via launch_persistent_context using a COPY of the real
    Chrome profile so the session cookies are available without locking
    the live profile or conflicting with open Chrome windows.
    """
...
def _try_locators(page: Page, xpaths: list,
    ...
    for xpath, label in xpaths:
        try:
            ...
            return loc
        except Exception:   # OPT-8: merged PWTimeout + Exception into one clause
            continue
    return None
...
def _build_text_xpaths(aria_label: str, field_type: str) -> tuple:
    """
    Build and CACHE the XPath probe list for a given (aria_label, field_type).
    OPT-5: fill_text_field is called ~221 times per run (13 fields × 17 meetings).
    Without caching it rebuilt 8 f-strings on every call. With lru_cache the
    list is built once per unique (label, type) pair and reused thereafter.
    Returns a tuple (hashable) so it can be cached.
    """
...
def fill_text_field(page: Page, aria_label: str, value: str,
    ...
    xpaths = _build_text_xpaths(aria_label, field_type)
...
def fill_email_field(page: Page, email_list,
    ...
    # OPT-9: build xpaths as a list only when aria-labelledby may prepend;
    # use a tuple (faster iteration) otherwise.
    base_xpaths = [
    ...
    # Attempt aria-labelledby resolution first — prepend if found
    try:
        ...
        if lbl_id:
            base_xpaths.insert(0, (f"//input[@aria-labelledby='{lbl_id}']", "aria-labelledby"))
    except Exception:
        pass

    loc = _try_locators(page, base_xpaths)
    if loc is None:
...
def verify_run_panel(page: Page, flow_name: str) -> None:
    ...
    print(f"  ✓ Run panel: '{loc.inner_text()}'" if loc else "  ⚠ Run panel not detected — continuing.")
...
def run_power_automate_flow(
    ...
    """
    Execute one Power Automate flow for the given meeting.

    OPT-2: All release-level constants (pa_url, URLs, display_release, etc.)
           are pre-computed once in create_teams_meeting and passed in —
           not recomputed on every meeting call.
    OPT-3: page.goto(pa_url) is called here per meeting so the PA flow list
           is fresh each time (PA single-page app — back navigation is unreliable).
    """
    print(f"  Subject : {flow_data['subject']}")
    print(f"  Date    : {format_date_display(meeting_date)}"
          f"  ({meeting_cfg['default_start_time']} - {meeting_cfg['default_end_time']})")
    print(f"  -> {pa_url}")

    # domcontentloaded is reliable for SPAs; networkidle never used (PA long-polls)
...
def create_teams_meeting(project_name: str, release_number: str,
    ...
    with sync_playwright() as pw:
        try:
            ...
            # ── OPT-2: pre-compute all release-level constants once ───────────
            releases      = config.get("releases", {})
            ...
            # ── OPT-6: use pre-extracted schedule dict, build flow_data here ──
            for name, meeting_cfg in meetings.items():
                ...
                # ── OPT-2: build flow_data once per meeting (not inside runner) ─
                ...
                retro_url = get_retro_url(config, release_number, name)
                ...
                flow_data = {
                    ...
                    "release_retro_url":      retro_url,
                }

                run_meetings.append((name, date, meeting_cfg, flow_data))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--profile",        default=None,
                        help="Path to Chrome user-data-dir (optional).\n"
                             "Default: auto-detected for your OS.\n"
                             "Find it: Chrome -> chrome://version -> Profile Path\n"
                             "(parent folder, not the 'Default' subfolder).\n"
