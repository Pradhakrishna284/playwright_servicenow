"""
Shared utilities for Teams Meeting HTTP automation.

Used by both create_teams_meeting.py and cancel_teams_meeting.py.
Do not add script-specific logic here.
"""

import argparse
import logging
import os
import time
from datetime import datetime

import requests
import yaml


# ── Logging ───────────────────────────────────────────────────────────────────
# Root logger is configured by the entry-point scripts (--verbose / --log-file).
# All modules use: log = logging.getLogger(__name__)
log = logging.getLogger(__name__)


# ── Display separators ────────────────────────────────────────────────────────
HR  = "─" * 70   # thin rule  (section dividers, per-meeting)
HR2 = "=" * 70   # thick rule (top/bottom of summary blocks)


# ── HTTP settings ─────────────────────────────────────────────────────────────
# The Power Automate "Create a Teams meeting" action has Time zone set to
# "India Standard Time" (confirmed in PA flow UI). So we send plain datetime
# strings with no Z and no offset — PA interprets them as IST directly.
#
#   start_datetime = "2026-05-19T12:30:00"  → PA creates meeting at 12:30 IST
#   end_datetime   = "2026-05-19T13:00:00"  → PA ends meeting   at 13:00 IST
HTTP_TIMEOUT   = 30    # seconds — per-request timeout
HTTP_RETRIES   = 3     # retry attempts on transient failures
RETRY_BACKOFF  = 2     # seconds between retries (doubles each attempt)

# HTTP status codes that are safe to retry
_RETRYABLE_CODES = {429, 500, 502, 503, 504}

_CONFIG_PATHS = {
    "TDR": "config/config_tdr.yaml",
    "CRE": "config/config_cre.yaml",
}

_PROJECT_FULL_NAMES = {
    "CRE": "Content Rate Extract",
    "TDR": "Tax Data Repository",
}


# ══════════════════════════════════════════════════════════════════════════════
#  Config loading
# ══════════════════════════════════════════════════════════════════════════════

def get_config_path(project_name: str) -> str:
    return _CONFIG_PATHS.get(project_name.upper(),
                             f"config/config_{project_name.lower()}.yaml")


def _resolve(value, lookup: dict, section: str):
    """Replace a '$name' string with the value from lookup; warn on miss."""
    if isinstance(value, str) and value.startswith("$"):
        key = value[1:]
        if key in lookup:
            return lookup[key]
        log.warning("Reference '%s' not found in %s — left unresolved.", value, section)
    return value


def load_config(project_name: str, config_file: str = None) -> tuple[dict, str]:
    """
    Load and fully expand a YAML project config.

    Returns (config_dict, resolved_path) so callers never need to re-derive
    the path themselves.

    Expansions applied in order for each meeting:
      1. time_slot ref  → default_start_time / default_end_time
      2. attendee group refs
      3. form field set ref

    Raises:
        FileNotFoundError: if the config file does not exist.
        ValueError: if a meeting entry in the YAML is not a dict.
    """
    path = config_file or get_config_path(project_name)
    if not os.path.exists(path):
        raise FileNotFoundError(f"Config file not found: '{path}'")

    with open(path, encoding="utf-8") as fh:
        config = yaml.safe_load(fh)

    attendee_groups = config.get("attendee_groups", {})
    form_field_sets = config.get("form_field_sets", {})
    time_slots      = config.get("time_slots", {})

    for name, meeting_cfg in config.get("meetings", {}).items():
        # Guard: each meeting must be a dict (catches YAML authoring mistakes)
        if not isinstance(meeting_cfg, dict):
            raise ValueError(
                f"Meeting '{name}' in config is not a mapping (got "
                f"{type(meeting_cfg).__name__}). Check the YAML indentation."
            )

        slot_ref = meeting_cfg.pop("time_slot", None)
        if slot_ref:
            slot = _resolve(slot_ref, time_slots, "time_slots")
            if isinstance(slot, dict):
                start = slot["start"]
                end   = slot["end"]
                # Validate slot ordering at load time so bad YAML is caught early
                if start >= end:
                    raise ValueError(
                        f"Meeting '{name}': time_slot start ({start}) is not "
                        f"before end ({end}). Check the YAML time_slots section."
                    )
                meeting_cfg["default_start_time"] = start
                meeting_cfg["default_end_time"]   = end
            else:
                log.warning("time_slot for '%s' could not be resolved.", name)

        meeting_cfg["required_attendees"] = _resolve(
            meeting_cfg.get("required_attendees", []), attendee_groups, "attendee_groups"
        )
        if "optional_attendees" in meeting_cfg:
            meeting_cfg["optional_attendees"] = _resolve(
                meeting_cfg["optional_attendees"], attendee_groups, "attendee_groups"
            )

        meeting_cfg["form_fields"] = _resolve(
            meeting_cfg.get("form_fields", []), form_field_sets, "form_field_sets"
        )

    log.info("Config loaded: '%s'", path)
    print(f"✓ Config loaded: '{path}'")
    return config, path


# ══════════════════════════════════════════════════════════════════════════════
#  Release / version helpers
# ══════════════════════════════════════════════════════════════════════════════

def normalize_release_number(project_name: str, release_number: str) -> str:
    prefix = project_name.upper() + " "
    return release_number if release_number.startswith(prefix) else prefix + release_number


def extract_version(release_number: str) -> str:
    parts = release_number.split()
    return parts[-1] if len(parts) > 1 else release_number


def decrement_version(version: str) -> str:
    """
    Return the version string for the previous month.

    Raises ValueError if the version string cannot be parsed, so callers get
    a clear error rather than silently using the wrong URL.
    """
    try:
        y, m, d = version.split(".")
        year, month = int(y), int(m)
        year, month = (year - 1, 12) if month == 1 else (year, month - 1)
        return f"{year}.{month:02d}.{d}"
    except Exception as exc:
        raise ValueError(f"Cannot decrement version '{version}': {exc}") from exc


def build_release_branch_name(version: str, release_branch_date: str) -> str:
    """
    Build the release branch name from the version and the Release Branch date.

    Format: release/<version-month zero-padded>.<yyyymmdd>-B

    The prefix is the version month itself:
      2026.06.00  Release Branch date 2026-05-20  →  release/06.20260520-B
      2026.12.00  Release Branch date 2026-12-16  →  release/12.20261216-B
    """
    try:
        month = int(version.split(".")[1])
        date_compact = release_branch_date.replace("-", "")
        return f"release/{month:02d}.{date_compact}-B"
    except Exception as exc:
        log.warning(
            "build_release_branch_name('%s', '%s'): %s", version, release_branch_date, exc
        )
        return ""


# ══════════════════════════════════════════════════════════════════════════════
#  Schedule lookup
# ══════════════════════════════════════════════════════════════════════════════

def get_version_schedule(config: dict, version: str) -> dict:
    """
    Return the flat {meeting_name: date_str} dict for a version.

    PyYAML parses bare ISO dates (YYYY-MM-DD) as datetime.date objects unless
    they are quoted. All values are normalised to strings here so the rest of
    the codebase can assume str throughout.
    """
    entry  = config.get("releases", {}).get("schedule", {}).get(version, {})
    nested = entry.get("meeting_dates")
    raw    = nested if nested is not None else entry
    # Normalise date values: datetime.date → "YYYY-MM-DD" string
    return {k: str(v) for k, v in raw.items() if not isinstance(v, dict)}


# ══════════════════════════════════════════════════════════════════════════════
#  Date formatting
# ══════════════════════════════════════════════════════════════════════════════

def format_date_display(date_str) -> str:
    if not date_str:
        return "N/A"
    try:
        d   = datetime.strptime(str(date_str), "%Y-%m-%d")
        day = d.day
        suffix = "th" if 11 <= day <= 13 else {1: "st", 2: "nd", 3: "rd"}.get(day % 10, "th")
        return f"{day}{suffix} {d.strftime('%b %Y')}"
    except ValueError:
        log.warning("Could not parse date '%s' for display — check YAML schedule.", date_str)
        return str(date_str)


# ══════════════════════════════════════════════════════════════════════════════
#  URL validation
# ══════════════════════════════════════════════════════════════════════════════

def validate_flow_url(url: str, field_name: str) -> None:
    """
    Raise ValueError if a flow URL looks wrong.

    Catches the most common mistake: pasting the Power Automate environment
    browse URL instead of the HTTP trigger URL.
    """
    if not url:
        return   # missing URL is handled separately by callers
    if not url.startswith("https://"):
        raise ValueError(
            f"'{field_name}' does not look like an HTTPS URL.\n"
            f"  Got: {url[:120]}\n"
            f"  Expected a URL starting with 'https://'"
        )
    # PA environment browse URLs are not trigger URLs
    if "make.powerautomate.com/environments" in url and "/flows" in url:
        raise ValueError(
            f"'{field_name}' looks like a Power Automate environment URL, "
            f"not an HTTP trigger URL.\n"
            f"  Got: {url[:120]}\n"
            f"  Open the flow in Power Automate, copy the URL from the "
            f"'When an HTTP request is received' trigger step."
        )


# ══════════════════════════════════════════════════════════════════════════════
#  Confirmation prompt
# ══════════════════════════════════════════════════════════════════════════════

def confirm_bulk_run(action: str, count: int, display_release: str) -> bool:
    """
    Prompt the user to confirm a bulk (non-filtered) live run.

    Returns True if the user confirms, False if they abort.
    Skipped when running in a non-interactive environment (e.g. CI).
    """
    if not os.isatty(0):
        # Non-interactive — proceed without prompting (CI/scheduled jobs)
        log.info("Non-interactive environment detected; skipping confirmation prompt.")
        return True

    print(f"\n  ⚠  About to {action} {count} flow(s) for {display_release}.")
    try:
        answer = input("  Confirm? [y/N] ").strip().lower()
    except EOFError:
        answer = ""
    if answer == "y":
        return True
    print("  Aborted.")
    return False


# ══════════════════════════════════════════════════════════════════════════════
#  --meeting filter warning  (shared by create and cancel)
# ══════════════════════════════════════════════════════════════════════════════

def warn_filter_no_match(
    meeting_filter: str,
    run_list: list,
    all_meetings: dict,
    version: str,
) -> None:
    """
    Print a helpful warning when --meeting matched nothing.

    Distinguishes between two cases:
      1. The name doesn't exist in config at all.
      2. The name exists but has no date for this version.
    """
    if not meeting_filter or run_list:
        return
    if meeting_filter not in all_meetings:
        print(f"  ⚠ WARNING: No meeting named '{meeting_filter}' exists in config.")
        print(f"  Run with --list-meetings to see available names.")
    else:
        print(f"  ⚠ WARNING: Meeting '{meeting_filter}' is defined but has no date")
        print(f"  in releases.schedule for version '{version}'. Nothing to run.")


# ══════════════════════════════════════════════════════════════════════════════
#  HTTP trigger — core
# ══════════════════════════════════════════════════════════════════════════════

def trigger_flow(trigger_url: str, payload: dict, dry_run: bool = False) -> bool:
    """
    POST the payload to the flow's HTTP trigger URL.

    Returns True on success (HTTP 2xx / 202 Accepted).
    Retries on transient failures (429, 5xx) with exponential backoff.

    Raises RuntimeError after all retry attempts are exhausted, or immediately
    on a non-retryable HTTP error.
    """
    if dry_run:
        log.debug("[DRY RUN] Would POST to: %s", trigger_url[:80])
        print(f"  [DRY RUN] Would POST to: {trigger_url[:80]}...")
        print(f"  [DRY RUN] Payload: {payload}")
        return True

    delay = RETRY_BACKOFF
    last_exc: Exception | None = None

    for attempt in range(1, HTTP_RETRIES + 1):
        try:
            resp = requests.post(
                trigger_url,
                json=payload,
                timeout=HTTP_TIMEOUT,
                headers={"Content-Type": "application/json"},
            )

            if resp.status_code in (200, 202):
                log.info("Flow triggered successfully (HTTP %s).", resp.status_code)
                print(f"  ✓ Flow triggered  (HTTP {resp.status_code})")
                return True

            if resp.status_code in _RETRYABLE_CODES and attempt < HTTP_RETRIES:
                log.warning(
                    "HTTP %s — retry %d/%d in %ds...",
                    resp.status_code, attempt, HTTP_RETRIES, delay,
                )
                print(f"  ⚠ HTTP {resp.status_code} — retry {attempt}/{HTTP_RETRIES} in {delay}s...")
                time.sleep(delay)
                delay *= 2
                continue

            # Non-retryable or final attempt
            raise RuntimeError(f"HTTP {resp.status_code}: {resp.text[:200]}")

        except requests.exceptions.Timeout as exc:
            last_exc = exc
            if attempt < HTTP_RETRIES:
                log.warning("Timeout — retry %d/%d in %ds...", attempt, HTTP_RETRIES, delay)
                print(f"  ⚠ Timeout — retry {attempt}/{HTTP_RETRIES} in {delay}s...")
                time.sleep(delay)
                delay *= 2
            else:
                raise RuntimeError(
                    f"Request timed out after {HTTP_TIMEOUT}s "
                    f"({HTTP_RETRIES} attempts)"
                ) from last_exc

        except requests.exceptions.ConnectionError as exc:
            last_exc = exc
            if attempt < HTTP_RETRIES:
                log.warning(
                    "Connection error — retry %d/%d in %ds...", attempt, HTTP_RETRIES, delay
                )
                print(f"  ⚠ Connection error — retry {attempt}/{HTTP_RETRIES} in {delay}s...")
                time.sleep(delay)
                delay *= 2
            else:
                raise RuntimeError(
                    f"Connection failed after {HTTP_RETRIES} attempts: {last_exc}"
                ) from last_exc

    return False  # unreachable, but satisfies type checkers


# ══════════════════════════════════════════════════════════════════════════════
#  Shared summary printer
# ══════════════════════════════════════════════════════════════════════════════

def _print_summary(
    title: str,
    display_release: str,
    run_list: list,
    skip_meetings: list,
    results: dict,
) -> None:
    """Print the final execution summary (shared by create and cancel scripts)."""
    print(f"\n{HR2}")
    print(f"  {title}  —  {display_release}")
    print(HR2)
    print(f"  Total available : {len(run_list) + len(skip_meetings)}")
    print(f"  Skipped         : {len(skip_meetings)}")
    print(f"  Executed        : {len(run_list)}")
    print(f"  Passed          : {results['success_count']}")
    print(f"  Failed          : {results['failed_count']}")
    if run_list:
        pct = results["success_count"] / len(run_list) * 100
        print(f"  Pass rate       : {pct:.1f}%")
    print(HR)
    for i, info in enumerate(results["passed_flows"], 1):
        print(f"  {i:2}. ✓ {info['flow_name']}  ({format_date_display(info['date'])})")
    for i, info in enumerate(results["failed_flows"], 1):
        print(f"  {i:2}. ✗ {info['flow_name']}  ({format_date_display(info['date'])})")
        print(f"        Error: {info['error']}")
    for i, (n, reason) in enumerate(skip_meetings, 1):
        print(f"  {i:2}. ⊘ {n}  ({reason})")
    print(f"{HR2}\n")


# ══════════════════════════════════════════════════════════════════════════════
#  Argument parser factory  (shared by both entry-point scripts)
# ══════════════════════════════════════════════════════════════════════════════

def build_arg_parser(description: str) -> "argparse.ArgumentParser":
    """
    Return an ArgumentParser pre-loaded with the arguments common to both
    create and cancel scripts.  Each script calls this, then may add its own
    script-specific arguments before calling parser.parse_args().
    """
    parser = argparse.ArgumentParser(
        description=description,
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument(
        "--project", required=True,
        help="Project name.\nChoices: CRE, TDR",
    )
    parser.add_argument(
        "--release-version", required=True,
        help="Release version number.\nExample: 2026.07.00",
    )
    parser.add_argument(
        "--config", default=None,
        help="Path to YAML config file (optional).\n"
             "Default: config/config_<project>.yaml",
    )
    parser.add_argument(
        "--dry-run", action="store_true",
        help="Print what would be sent without making HTTP calls.\n"
             "Useful for verifying config before a real run.",
    )
    parser.add_argument(
        "--meeting", default=None,
        help="Run only the named meeting (exact match).\n"
             'Example: --meeting "UAT Deployment"',
    )
    parser.add_argument(
        "--list-versions", action="store_true",
        help="List all release versions available in the config and exit.",
    )
    parser.add_argument(
        "--list-meetings", action="store_true",
        help="List all meeting names defined in the config and exit.",
    )
    parser.add_argument(
        "--yes", "-y", action="store_true",
        help="Skip the confirmation prompt for bulk runs (non-dry-run without --meeting).",
    )
    parser.add_argument(
        "--verbose", action="store_true",
        help="Enable DEBUG-level logging (includes full tracebacks on failures).",
    )
    parser.add_argument(
        "--log-file", default=None, metavar="PATH",
        help="Write log output to this file in addition to stdout.",
    )
    return parser


def configure_logging(verbose: bool = False, log_file: str = None) -> None:
    """
    Configure the root logger.

    - INFO level by default; DEBUG when --verbose is set.
    - Always writes to stdout via StreamHandler.
    - Optionally writes to a file (--log-file).
    """
    level = logging.DEBUG if verbose else logging.INFO
    fmt   = "%(asctime)s  %(levelname)-8s  %(name)s  %(message)s"
    handlers: list[logging.Handler] = [logging.StreamHandler()]
    if log_file:
        handlers.append(logging.FileHandler(log_file, encoding="utf-8"))
    logging.basicConfig(level=level, format=fmt, handlers=handlers)


# ══════════════════════════════════════════════════════════════════════════════
#  Discovery helpers  (--list-versions / --list-meetings)
# ══════════════════════════════════════════════════════════════════════════════

def print_available_versions(project_name: str, config_file: str = None) -> None:
    """Print all release versions defined in releases.schedule."""
    config, path = load_config(project_name, config_file)
    versions = sorted(config.get("releases", {}).get("schedule", {}).keys())
    print(f"\nAvailable versions for {project_name.upper()}  ({path}):")
    if versions:
        for v in versions:
            print(f"  {v}")
    else:
        print("  (none found — check releases.schedule in the config)")


def print_available_meetings(project_name: str, config_file: str = None) -> None:
    """Print all meeting names defined in meetings."""
    config, path = load_config(project_name, config_file)
    meetings = list(config.get("meetings", {}).keys())
    print(f"\nMeetings defined in config  ({path})  [{len(meetings)} total]:")
    for i, name in enumerate(meetings, 1):
        print(f"  {i:2}. {name}")