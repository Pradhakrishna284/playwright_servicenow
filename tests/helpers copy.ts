 ...
 *     if the credentials are set, then waiting for PingID MFA.
...
export async function handleSSOExpiry(page: Page): Promise<void> {
  ...
  if (ssoUsername && ssoPassword) {
    ...
    await page.waitForLoadState('domcontentloaded');
    console.log('✓ Credentials submitted — waiting for PingID MFA approval on your device…');
  } else {
    console.log('⚠ Session expired — SSO_USERNAME / SSO_PASSWORD not in .env.');
    console.log('👉 Please complete the SSO login (username, password + PingID MFA) in the browser window…');
  }

  // Wait up to 2 min for the user to approve PingID and land on ServiceNow.
