...
// Must match the path used in sso_setup.ts and playwright.config.ts storageState.
...
export async function fillAutosuggest(
  ...
  // No hardcoded pre-wait — dropdown.waitFor() below handles the AJAX response.
  ...
  await option.click();
  await new Promise(r => setTimeout(r, 600)); // allow ServiceNow to update dependent fields
...
export async function fillMainForm(
  ...
  for (const group of testData.additionalApprovalGroups) {
    ...
    await groupInput.type(group, { delay: 20 });
...
export async function createRiskAssessmentTask(
  ...
  // Use DOM-level element.click() as primary — this fires the full onclick chain
  // (which sets up any required window context before calling invokeAssessment()).
  // Calling invokeAssessment() directly bypasses that setup and the popup may not open.
  // Fall back to invokeAssessment() only if element.click() throws.
  const gsftFrame = page.frames().find(f => f !== page.mainFrame());
  if (gsftFrame) {
    try {
      await btn.evaluate((el: any) => el.click());
      console.log('✓ Clicked Risk Assessment (element.click)');
    } catch {
      // Fallback: call invokeAssessment() directly in the frame context
      try {
        ...
        console.log('✓ Clicked Risk Assessment (invokeAssessment fallback)');
      } catch {
        await btn.click({ force: true });
        console.log('✓ Clicked Risk Assessment (force click fallback)');
      }
    }
  } else {
