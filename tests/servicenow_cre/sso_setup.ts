...
// ─── paths ───────────────────────────────────────────────────────────────────
// sso_setup.ts lives at the project root alongside playwright.config.ts.
// Both resolve auth_servicenow.json relative to __dirname (no subdir needed).
...
setup('Authenticate via SSO/PingID', async ({ page }) => {
  ...
  try {
    ...
    if (missing.length) {
      ...
      console.log(`✓ Session saved to ${AUTH_FILE} (valid for 2 hours)`);
