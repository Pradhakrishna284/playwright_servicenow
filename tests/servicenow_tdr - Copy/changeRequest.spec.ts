    ...
    test(`[${configKey}] CR Creation and CTask Management`, async ({ page }) => {
      ...
      await test.step('8. Create Change Tasks', async () => {
        ...
        for (let i = 0; i < ctaskConfigs.length; i++) {
          ...
          await page.waitForTimeout(T_TAB_SETTLE);  // brief wiring buffer after New button appears

          // Reuse same iframe — no navigation between waitFor and click.
          await clickElement(page, iframe.locator('button').filter({ hasText: /^New$/ }).first(), 'New CTask button');

          // Wait for the CTask form URL — createCtask waits for the submit
          // button internally, so no additional settle time is needed here.
      ...
      // ── 11. Request Approval ─────────────────────────────────────────────────
        ...
        // Use explicit timeout + reload (same as the proven riskmanagement approach).
        ...
        // state form after navigation; 5s was too short, T_ELEMENT gives enough headroom.
        ...
        try {
          ...
          console.warn(`⚠ Request Approval button still visible — CR may have extra approval group requirements blocking the transition. Verify ${configKey} CR manually in ServiceNow.`);
        }
