...
// ─── paths ───────────────────────────────────────────────────────────────────
// sso_setup.ts and playwright.config.ts both live in servicenow_tdr/,
// so AUTH_FILE and the config's storageState must use the same __dirname
// (no '..' prefix). Writing to a parent folder caused a path mismatch that
// forced a fresh SSO login on every run despite the session still being valid.
const AUTH_FILE = path.join(__dirname, 'auth_servicenow.json');
const LOCK_FILE = path.join(__dirname, 'auth_servicenow.lock');

// ─── timing constants ────────────────────────────────────────────────────────
const AUTH_MAX_AGE_MS  = 100 * 60 * 1000; // 100 min — 20 min buffer before PingID 2h expiry
                                           // (2h threshold caused borderline sessions to pass the
                                           //  freshness check but expire before tests start)
...
setup('Authenticate via SSO/PingID', async ({ page }) => {
  ...
  try {
    ...
    if (missing.length) {
      ...
      await page.waitForURL(/trenterprise\.service-now\.com/, { timeout: 120_000, waitUntil: 'domcontentloaded' });
      await page.waitForLoadState('domcontentloaded');
    ...
    // Wait for PingID MFA approval (manual step — approve on your phone/device).
    // 120 s allows time for production PingID push-notification approval.
    console.log('⏳ Waiting for PingID approval on your device (up to 2 minutes)…');
    await page.waitForURL(/trenterprise\.service-now\.com/, { timeout: 120_000, waitUntil: 'domcontentloaded' });
    await page.waitForLoadState('domcontentloaded');
    console.log('✓ SSO authentication successful');

    await expect(page).toHaveURL(/trenterprise\.service-now\.com/);
